        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8">
          ...
          <div className="space-y-4">
            <div className="flex items-center">
              <img src="https://quest-media-storage-bucket.s3.us-east-2.amazonaws.com/1751268017774-Silahub%20Technologies%20-%20White%20Primary%20Logo.png" alt="Silahub Technologies" className="h-12 w-auto" />
            </div>
            ...
            <div className="flex space-x-3">
              {socialLinks.map((social)=> (
                <a key={social.name} href={social.url} target="_blank" rel="noopener noreferrer" className="text-gray-400 hover:text-silahub-secondary transition-colors p-2 bg-gray-800 rounded-lg hover:bg-gray-700" aria-label={`Follow us on ${social.name}`} >
            ...
            <div className="mt-2">
              <a href="https://www.tiktok.com/@silahub.technolog" target="_blank" rel="noopener noreferrer" className="text-gray-400 hover:text-silahub-secondary transition-colors text-sm flex items-center space-x-2" aria-label="Follow us on TikTok" >
          ...
          <div>
            ...
            <ul className="space-y-2">
              {services.map((service,index)=> (
                <li key={index}>
                  <Link to={service.href} onClick={handleLinkClick} className="text-gray-300 hover:text-white transition-colors text-sm" >
                    {service.name}
                  </Link>
                </li>
              ))}
          ...
          <div>
            ...
            <ul className="space-y-2">
              {quickLinks.map((link,index)=> (
                <li key={index}>
                  <Link to={link.href} onClick={handleLinkClick} className="text-gray-300 hover:text-white transition-colors" >
                    {link.name}
                  </Link>
                </li>
              ))}
            ...
            <div className="mt-4 pt-4 border-t border-gray-800">
              <Link to="/admin" onClick={handleLinkClick} className="text-gray-500 hover:text-gray-400 transition-colors text-xs" >
                Admin Dashboard
              </Link>
            </div>
          </div>
