...
const Contact=()=> {
  ...
  React.useEffect(()=> {
    ...
    if (metaDescription) {
      metaDescription.setAttribute('content','Contact Silahub Technologies for your free marketing consultation. Call 825-288-8332 or email hello@silahub.com. Serving Calgary home service businesses.');
    }
  },[]);

  const contactInfo=[
    {icon: FiPhone,title: 'Phone',details: '825-288-8332',description: 'Call us directly for immediate assistance',action: 'tel:8252888332'},
  ...
  const faqs=[
    {question: 'How long does it take to see results?',answer: 'Most clients start seeing increased website traffic within 30 days. For SEO,significant ranking improvements typically occur within 3-6 months. Google Ads can generate leads immediately once campaigns are optimized.'},
    {question: 'Do you work with businesses outside Calgary?',answer: 'Yes! While we specialize in Calgary businesses,we work with home service companies across Alberta and select markets in the USA.'},
    {question: 'What makes you different from other agencies?',answer: 'We exclusively focus on home service businesses,so we understand your unique challenges. Our technical background and proven track record with 500+ clients sets us apart.'},
    {question: 'Do you require long-term contracts?',answer: 'No long-term contracts required. We believe our results should speak for themselves. Most clients stay with us because of the value we provide.'},
    {question: 'How much does your service cost?',answer: 'Our pricing varies based on your specific needs and goals. We offer customized packages starting from $500/month. Contact us for a free audit and personalized quote.'}
  ];
    ...
    <>
              ...
              <div className="pt-8 border-t border-gray-200">
                ...
                <div className="flex flex-wrap gap-3">
                  {socialLinks.map((social)=> (
                    <a key={social.name} href={social.url} target="_blank" rel="noopener noreferrer" className="bg-gray-100 p-3 rounded-lg hover:bg-primary-100 transition-colors group" aria-label={`Follow us on ${social.name}`} >
                      <SafeIcon icon={social.icon} className="w-5 h-5 text-gray-600 group-hover:text-primary-600 transition-colors" />
                    </a>
                  ))}
                  {/* TikTok Link */}
                  <a href="https://www.tiktok.com/@silahub.technolog" target="_blank" rel="noopener noreferrer" className="bg-gray-100 p-3 rounded-lg hover:bg-primary-100 transition-colors group flex items-center justify-center" aria-label="Follow us on TikTok" >
      ...
      <section className="py-20 bg-gray-50">
        <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <div className="space-y-6">
            {faqs.map((faq,index)=> (
              <motion.div key={index} initial={{opacity: 0,y: 20}} whileInView={{opacity: 1,y: 0}} viewport={{once: true}} transition={{delay: index * 0.1}} className="bg-white rounded-xl shadow-lg p-6" >
        ...
        <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 text-center">
          <motion.div initial={{opacity: 0,y: 20}} whileInView={{opacity: 1,y: 0}} viewport={{once: true}} >
            ...
            <div className="flex flex-col sm:flex-row gap-4 justify-center">
              <a href="tel:8252888332" className="bg-white text-primary-600 px-8 py-4 rounded-lg font-semibold hover:bg-gray-100 transition-colors inline-flex items-center justify-center space-x-2" >
