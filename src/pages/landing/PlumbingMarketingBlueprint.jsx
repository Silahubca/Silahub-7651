...
const { FiDownload, FiCheck, FiTrendingUp, FiDollarSign, FiDroplet, FiLoader } = FiIcons;
...
const PlumbingMarketingBlueprint = () => {
  ...
  useEffect(() => {
    if (formData.monthlyRevenue && formData.currentLeads && formData.avgJobValue) {
      ...
      // Assumptions for Plumbing
      const conversionRate = 0.32; // 32% closing rate
      const marketingSpend = 2200;
      
      const projectedLeads = leads * 3.8; // 380% increase
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const pdfData = {
        ...
        type: 'plumbing',
        filename: 'plumbing-marketing-blueprint.pdf',
      ...
      await generateBlueprintPDF('plumbing', formData, roasResults, 'plumbing-marketing-blueprint.pdf');

      alert('Thank you! Your Plumbing Marketing Blueprint has been downloaded to your device. Check your downloads folder.');
    } catch (error) {
      console.error(error);
          ...
          <div className="grid lg:grid-cols-2 gap-12 items-center">
            <motion.div initial={{ opacity: 0, x: -50 }} animate={{ opacity: 1, x: 0 }}>
              <div className="inline-flex items-center space-x-2 bg-white/20 px-4 py-2 rounded-full text-sm font-medium mb-6">
                ...
                <span>For Plumbing Businesses</span>
              </div>
              <h1 className="text-4xl lg:text-5xl font-bold mb-6">
                Plumbing Marketing Blueprint
              </h1>
              <p className="text-xl text-blue-100 mb-8 leading-relaxed">
                Stop competing on price and become the go-to plumber for emergency and high-value jobs. Master local SEO and lead generation.
              </p>
              <div className="flex flex-wrap gap-4 text-sm font-medium">
                <div className="flex items-center bg-white/10 px-4 py-2 rounded-lg">
                  <SafeIcon icon={FiCheck} className="w-5 h-5 mr-2 text-blue-200" />
                  High-Value Jobs
                </div>
                <div className="flex items-center bg-white/10 px-4 py-2 rounded-lg">
                  <SafeIcon icon={FiCheck} className="w-5 h-5 mr-2 text-blue-200" />
              ...
              <form onSubmit={handleSubmit} className="space-y-4">
                ...
                {roasResults && (
                  <div className="bg-blue-50 border border-blue-200 rounded-lg p-4 mt-4">
                    <p className="text-sm font-semibold text-blue-900 mb-2">Projected Growth:</p>
                ...
                <button
                  ...
                  className="w-full bg-blue-600 text-white py-3 rounded-lg font-bold hover:bg-blue-700 transition-colors flex items-center justify-center space-x-2 disabled:opacity-50"
                >
                  {isSubmitting ? (
                    <>
