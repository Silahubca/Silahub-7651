...
const RoofingGrowthBlueprint = () => {
  ...
  useEffect(() => {
    if (formData.monthlyRevenue && formData.currentLeads && formData.avgJobValue) {
      ...
      // Assumptions for Roofing
      const conversionRate = 0.20; // 20% closing rate
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const pdfData = {
        ...
        type: 'roofing',
        filename: 'roofing-growth-blueprint.pdf',
      ...
      await generateBlueprintPDF('roofing', formData, roasResults, 'roofing-growth-blueprint.pdf');

      alert('Thank you! Your Roofing Growth Blueprint has been downloaded to your device. Check your downloads folder.');
    } catch (error) {
      console.error(error);
          ...
          <div className="grid lg:grid-cols-2 gap-12 items-center">
            <motion.div initial={{ opacity: 0, x: -50 }} animate={{ opacity: 1, x: 0 }}>
              ...
              <h1 className="text-4xl lg:text-5xl font-bold mb-6">
                Roofing Growth Blueprint
              </h1>
              <p className="text-xl text-gray-200 mb-8 leading-relaxed">
                Stop relying on storm chasers. Build your own exclusive lead generation system and target high-value commercial roofing projects.
              </p>
              <div className="flex flex-wrap gap-4 text-sm font-medium">
                <div className="flex items-center bg-white/10 px-4 py-2 rounded-lg">
                  <SafeIcon icon={FiCheck} className="w-5 h-5 mr-2 text-gray-300" />
                  Exclusive Leads
                </div>
                <div className="flex items-center bg-white/10 px-4 py-2 rounded-lg">
                  <SafeIcon icon={FiCheck} className="w-5 h-5 mr-2 text-gray-300" />
              ...
              <form onSubmit={handleSubmit} className="space-y-4">
                ...
                {roasResults && (
                  <div className="bg-gray-50 border border-gray-200 rounded-lg p-4 mt-4">
                    <p className="text-sm font-semibold text-gray-900 mb-2">Projected Growth:</p>
                ...
                <button
                  ...
                  className="w-full bg-gray-800 text-white py-3 rounded-lg font-bold hover:bg-gray-700 transition-colors flex items-center justify-center space-x-2 disabled:opacity-50"
                >
                  {isSubmitting ? (
                    <>
